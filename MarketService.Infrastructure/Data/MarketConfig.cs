...
public class MarketOutcomeConfig : IEntityTypeConfiguration<MarketOutcome>
{
    public void Configure(EntityTypeBuilder<MarketOutcome> b)
    {
        b.HasKey(x => x.Id);
        b.Property(x => x.Label).IsRequired().HasMaxLength(32);

        b.HasIndex(x => new { x.MarketId, x.OutcomeIndex }).IsUnique();
...
public class MarketResolutionConfig : IEntityTypeConfiguration<MarketResolution>
{
    public void Configure(EntityTypeBuilder<MarketResolution> b)
    {
        b.HasKey(x => x.Id);

        b.Property(x => x.ResolveTxSignature).IsRequired().HasMaxLength(128);

        // 1:1 Market -> Resolution
        b.HasIndex(x => x.MarketId).IsUnique();

        b.HasOne(x => x.Market)
            .WithOne(m => m.Resolution)
            .HasForeignKey<MarketResolution>(x => x.MarketId)
            .OnDelete(DeleteBehavior.Cascade);
    }
}